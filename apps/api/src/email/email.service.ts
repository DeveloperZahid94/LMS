...
export class EmailService {
  ...
  async send(opts: { tenantId: string; to: string; subject: string; html: string }): Promise<SendResult> {
    ...
    try {
      if (cfg.provider === 'BREVO') {
        return await this.sendBrevo(cfg.brevoApiKey ?? '', from, opts.to, opts.subject, opts.html);
      }
      if (cfg.provider === 'SENDGRID') {
        return await this.sendSendGrid(cfg.sendgridApiKey ?? '', from, opts.to, opts.subject, opts.html);
      }
  ...
  private async sendBrevo(apiKey: string, from: { email: string; name: string }, to: string, subject: string, html: string): Promise<SendResult> {
    ...
    return { ok: false, status: res.status, error: body.slice(0, 300), provider: 'BREVO' };
  }

  private async sendSendGrid(apiKey: string, from: { email: string; name: string }, to: string, subject: string, html: string): Promise<SendResult> {
